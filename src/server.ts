...
export default class Service {
  ...
  async getLabels(params?: RequestParameters) {
    const [err, res] = await to(
      this.octokit.request(ApiMap['getLabels'], {
  ...
  async createLabel(params?: RequestParameters) {
    const [err, res] = await to(
      this.octokit.request(ApiMap['createLabel'], {
        owner: this.config.user,
        repo: this.config.repo,
        ...params,
      })
  ...
  async deleteLabel(params?: RequestParameters) {
    const [err, res] = await to(
      this.octokit.request(ApiMap['deleteLabel'], {
        owner: this.config.user,
        repo: this.config.repo,
        ...params,
      })
  ...
  async updateLabel(params?: RequestParameters) {
    const [err, res] = await to(
      this.octokit.request(ApiMap['updateLabel'], {
        owner: this.config.user,
        repo: this.config.repo,
        ...params,
      })
  ...
  async getIssues(params?: RequestParameters) {
    const [err, res] = await to(
      this.octokit.request(ApiMap['getIssues'], {
        owner: this.config.user,
        repo: this.config.repo,
        ...params,
        per_page: 20,
  ...
  async updateIssue(params?: RequestParameters) {
    const [err, res] = await to(
      this.octokit.request(ApiMap['updateIssue'], {
        owner: this.config.user,
        repo: this.config.repo,
        ...params,
      })
  ...
  async createIssue(params?: RequestParameters) {
    const [err, res] = await to(
      this.octokit.request(ApiMap['createIssue'], {
        owner: this.config.user,
        repo: this.config.repo,
        ...params,
      })
  ...
  async uploadImage(params: RequestParameters) {
    const [err, res] = await to(
      this.octokit.request(ApiMap['uploadImage'], {
        owner: this.config.user,
        repo: this.config.repo,
